...
text=Text(janela, height=12)
text.grid(row=0,column=0)

janela.title('emissão de certificado')

def select_file():
    types=(('text file', '*.xlsx'),('ALL files', '*.*'))
    f = fd.askopenfile(title="open a file", initialdir='C:/', filetypes=types)
    print(f)

    text.insert('1.0',f.read())
    workbook_alunos = f
    sheet_alunos = workbook_alunos


btn1 = Button(janela, text="open file", command=select_file)
btn1.grid(row=0,column=0)



botao = Button(janela, text='Gerar', command=gerar_certificado)
